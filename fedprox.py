...
if __name__ == "__main__":
    ...
    if not Path(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox").exists():
        os.makedirs(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox")
    
    json.dump(local_loss_record,        open(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox/local_loss_record.json", "w"),         cls=NumpyEncoder)
    json.dump(local_acc_bfag_record,    open(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox/local_acc_bfag_record.json", "w"),     cls=NumpyEncoder)
    json.dump(local_acc_afag_record,    open(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox/local_acc_afag_record.json", "w"),     cls=NumpyEncoder)
    json.dump(global_cfmtx_record,      open(f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox/global_cfmtx_record.json", "w"),       cls=NumpyEncoder)
    if args.save_model:
        print("Saving model ...", end="")
        torch.save(global_model.state_dict(), f"{args.log_folder}/{args.idx_folder}/E{epochs}/R{args.round}/fedprox/global_model.pth")
        print("Done!")